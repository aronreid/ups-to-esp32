...
Written 2026-09-21, against `v0.05`.

**The hardware is ordered and frozen.** No change to the PCB is possible, so
every item below is either already correct or has to be solved in firmware.
That is the lens this page should now be read through: it is not a list of
things to fix on the board, it is a list of things firmware must get right the
first time a board is powered.

Re-verified against the design that was actually sent for manufacture:

```
  BOARD_PIN_VBUS_EN      GPIO4  -> VBUS_EN       pulled to GND by R11 (100k)
  BOARD_PIN_VBUS_FAULT   GPIO5  -> VBUS_FAULT    pulled to +3V3 by R12 (10k)
  BOARD_PIN_LED_STATUS   GPIO48 -> LED_STAT
  BOARD_PIN_LED_FAULT    GPIO47 -> LED_FAULT
  BOARD_PIN_I2C_SDA/SCL  GPIO8/9
  BOARD_PIN_FACTORY_BTN  GPIO0  -> IO0           pulled to +3V3 by R4 (10k)
  IO3, IO45, IO46 left unconnected
  gerbers plotted from this exact board (476239b1a8ababfb)
```

Two of those are load-bearing for firmware. **R11 pulls VBUS_EN low**, so VBUS
is off from the instant power is applied until firmware asserts it -- a UPS
cannot be half-powered by a board that has not booted. **R12 pulls VBUS_FAULT
high**, so an idle FLG reads "no fault", which is what `board_vbus_fault()`
assumes.
...
## What can still be fixed, and what cannot

Firmware can still change: pin *roles*, timing, the VBUS cycle length
(`CONFIG_UPSA_VBUS_CYCLE_MS`), how FLG is interpreted, LED patterns, and every
policy decision above. If the AP2171W turns out to assert FLG in a way this
firmware misreads, that is a firmware fix.

What cannot change: which GPIO goes where, the 1.5 A current limit, the 2 A
polyfuse, the absence of an ILIM resistor, and the single-sided placement. If
any of those is wrong it is a Rev B, not a patch.
