...
**7. Every target image must exist, and differ from its siblings.** Identical
binaries mean the board configuration did not apply, so one target has the wrong
pin map — silent until hardware. Rev A is the only target since the Freenove
devkit was dropped on 2026-09-25, so this has nothing to compare today; it
comes back with Rev B.
