...
**5. Never delete a published release.** Supersede it. Deleting one that a
device has installed leaves that device reporting a version no release matches,
offering the "new" lower version forever. This was free three times here only
because no device had ever installed one.

**6. The image version must equal the tag.** CI fails otherwise. They are
