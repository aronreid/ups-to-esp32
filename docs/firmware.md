...
`netmgr` advertises `ups-adaptor.local` over mDNS once an address is acquired,
plus `_http._tcp` on 80 and `_nut._tcp` on 3493. This is the normal way to reach
...
**optional OLED** (IP, battery, runtime, load, fault), and **two plain LEDs**.
...
|---|---|---|
| Booting | solid | off |
| Provisioning AP up | fast blink | off |
| Connecting or retrying | slow blink | off |
| Online, UPS present | solid | off |
| Online, no UPS | solid | slow blink |
| VBUS fault | off | solid |

`board_status_set` takes a semantic state, not pins, so the LED scheme can
change without touching a caller. On the devkit the calls compile to a log line;
the patterns are verified on Rev A.

## Optional OLED

Gated on `CONFIG_UPSA_HAVE_OLED`, default off. When disabled the whole component
