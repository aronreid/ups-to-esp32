# UPS to ESP32 Module firmware architecture
...
2. **No GPIO numbers outside `components/board`.** Rev A is the reference
 target and the only one built. (Bring-up ran on a Freenove devkit, removed
 once Rev A existed.) A future revision is a new block in `board.h`.
...
| Command | Behaviour |
|---|---|
| `VER` | `UPS to ESP32 Module <version>` |
| `NETVER` | `1.2` |
...
First boot with no stored credentials brings up SoftAP `ups-esp32-XXXX` with
a captive-portal redirect, so setup needs only a browser. Pressing BOOT five
times within four seconds clears credentials and restarts into setup, which is
the recovery path for a device given the wrong network. Holding BOOT at
power-on cannot do it -- GPIO0 is the download strap, so the ROM flasher runs
instead -- and a long hold is ruled out too, because a serial terminal raising
DTR alone holds GPIO0 low through the auto-reset transistor.
...
`GET /metrics` is a Prometheus scrape target in the text exposition format,
rendered by `components/webui/metrics.c` and streamed line by line, so its size
never has to fit a buffer.

UPS readings use **nut_exporter's names**, checked against its README: every
NUT variable as `network_ups_tools_<name, dots as underscores>` with a `ups`
label, the status as `network_ups_tools_ups_status{flag="OL"}` with each flag
present as 0 or 1, and `network_ups_tools_device_info` carrying mfr, model,
serial and USB IDs. Dashboards and alerts written against nut_exporter should
work with Prometheus scraping the board directly. Only the seven flags this
firmware can set are exported (OL, OB, LB, RB, CHRG, DISCHRG, OVER);
nut_exporter can list others, which would be permanently zero here.

The board's own health is under `ups_esp32_`: `ups_attached`, `build_info`,
`uptime_seconds`, `crashes_total`, `heap_free_bytes`, `heap_min_free_bytes`,
`wifi_rssi_dbm`, `nut_clients`.

A value the UPS does not report is **absent**, never 0 -- the same rule as NUT
and `/api/status`. A missing series is honest; a battery charge of zero pages
someone. `tools/test/test_metrics.py` renders three boards on the host and
checks the format (with `prometheus_client`'s own parser when installed), the
naming, and the absences.

```yaml
scrape_configs:
  - job_name: ups
    static_configs:
      - targets: ["ups-esp32-XXXX.local:80"]
```

## Discovery

`netmgr` advertises `ups-esp32-XXXX.local` over mDNS once an address is acquired
...
header takes a standard module as seated (`UPSA_BOARD_OLED_OK`). Rev A's does
not -- it is wired in reverse -- so on Rev A the display exists only as a bench
build, `UPSA_REVA_OLED_VCC_FIRST`, for a module labelled VCC, GND, SCL, SDA: its
power pins match J3 and the display's SDA/SCL are swapped to suit
(`BOARD_OLED_SDA`/`_SCL` in `board.h`). `release.yml` refuses a release with it
set. Rev B is to take a standard module and enable the display normally. When disabled the whole component
...
**BOOT at runtime.** A press wakes a dimmed screen, and on a lit screen flips
to the other page. Five presses within four seconds also clear Wi-Fi settings.
