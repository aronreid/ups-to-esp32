...
populated. That keeps the option live for bench debugging and for field
diagnostics on a small run: plug a $2 module into any board, set
`CONFIG_UPSA_HAVE_OLED=y`, and it works. Boards shipped without the module cost
...
reach the web UI without hunting through a DHCP table. mDNS (`ups-adaptor.local`)
