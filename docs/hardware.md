# ups-adaptor Rev A hardware specification
...
module. Firmware therefore refuses a display build for Rev A
(`UPSA_BOARD_OLED_OK` in Kconfig, and an `#error` in `board.h`); the header
order is fixed for Rev B, whose target will enable it. Boards shipped without the module cost
...
reach the web UI without hunting through a DHCP table. mDNS (`ups-adaptor-XXXX.local`)
