...
/* TODO(M0): usb_host_install(), client event loop, device open on connect.
 * TODO(M0): GET_DESCRIPTOR(Report) -> hid_parse_report_descriptor().
...
static void ups_task(void *arg)
{
    ...
    for (;;) {
        ...
        while (s_cmd_q && xQueueReceive(s_cmd_q, &req, 0) == pdTRUE) {
            esp_err_t res = s_dev ? execute_cmd(&req) : ESP_ERR_INVALID_STATE;
