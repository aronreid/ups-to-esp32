...
/* After restoring VBUS, how long to wait for a bus-powered UPS to report that
 * it went away. A self-powered one never does; see do_recover(). */
#define VBUS_GONE_WAIT_MS     1500
...
/* Legacy Tripp Lite (09ae:0001): a vendor protocol instead of HID Power
 * Device, answered on the interrupt IN endpoint. See tripplite_legacy.h. */
static bool                     s_legacy;
static uint8_t                  s_intr_ep;          /* 0 if the interface has none */
static uint16_t                 s_intr_mps;
static SemaphoreHandle_t        s_intr_done;
static tl_config_t              s_tl;
...
static bool find_hid_interface(const usb_config_desc_t *cfg)
{
    ...
    bool in_hid_iface = false, found = false;

    s_intr_ep = 0;
    s_intr_mps = 0;
    while ((d = usb_parse_next_descriptor(d, cfg->wTotalLength, &offset)) != NULL) {
        if (d->bDescriptorType == USB_B_DESCRIPTOR_TYPE_INTERFACE) {
            /* The first HID interface is the one used; a second one after it
             * ends the search rather than replacing it. */
            if (found) break;
        ...
        } else if (in_hid_iface && d->bDescriptorType == USB_B_DESCRIPTOR_TYPE_ENDPOINT) {
            /* The endpoint descriptors follow the HID descriptor, so the scan
             * carries on past it. Only the legacy Tripp Lite path reads this
             * endpoint; HID Power Devices are polled with GET_REPORT. */
            const usb_ep_desc_t *ep = (const usb_ep_desc_t *)d;
            if ((ep->bEndpointAddress & 0x80) && (ep->bmAttributes & 0x03) == 0x03 && !s_intr_ep) {
                s_intr_ep = ep->bEndpointAddress;
                s_intr_mps = ep->wMaxPacketSize & 0x7FF;
            }
        }
    }
    return found;
...
/* Blocking interrupt IN read. Returns bytes read, or -1 on timeout or error.
 *
 * A timed-out transfer is still owned by the USB stack. It is CANCELLED --
 * halt, flush, clear -- and its completion awaited before it is freed; freeing
 * a transfer the stack still holds is how this driver once corrupted the heap
 * on unplug. */
static int intr_in(uint8_t *out, size_t cap, int timeout_ms)
{
    if (!s_intr_ep) return -1;
    size_t mps = s_intr_mps ? s_intr_mps : TL_PKT;
    size_t want = mps;                      /* an IN transfer is whole packets */
    while (want < cap) want += mps;

    usb_transfer_t *xfer = NULL;
    if (usb_host_transfer_alloc(want, 0, &xfer) != ESP_OK) return -1;
    xfer->device_handle    = s_dev;
    xfer->bEndpointAddress = s_intr_ep;
    xfer->num_bytes        = want;
    xfer->callback         = ctrl_cb;
    xfer->context          = s_intr_done;
    xSemaphoreTake(s_intr_done, 0);         /* drop any stale completion */
    ...
    for (int waited = 0; waited < timeout_ms; waited += 10) {
        usb_host_client_handle_events(s_client, pdMS_TO_TICKS(10));
        if (xSemaphoreTake(s_intr_done, 0) == pdTRUE) { done = true; break; }
    }
    if (!done) {
        usb_host_endpoint_halt(s_dev, s_intr_ep);
        usb_host_endpoint_flush(s_dev, s_intr_ep);
        for (int waited = 0; waited < 500 && !done; waited += 10) {
            usb_host_client_handle_events(s_client, pdMS_TO_TICKS(10));
            done = xSemaphoreTake(s_intr_done, 0) == pdTRUE;
        }
        usb_host_endpoint_clear(s_dev, s_intr_ep);
        if (!done) {
            /* Never completed even when cancelled: leak it rather than free
             * memory the stack may still write to. */
            ESP_LOGE(TAG, "interrupt transfer would not cancel; leaking %u bytes",
                     (unsigned)want);
            return -1;
        }
        usb_host_transfer_free(xfer);
    ...
    if (xfer->status == USB_TRANSFER_STATUS_COMPLETED) {
        got = xfer->actual_num_bytes;
        if (got > (int)cap) got = (int)cap;
        memcpy(out, xfer->data_buffer, got);
    }
    usb_host_transfer_free(xfer);
    return got;
}

/* One legacy command and its reply. The UPS answers on the interrupt endpoint
 * a moment later, and echoes its PREVIOUS reply if it did not take this
 * command, so a reply counts only if it starts with this command's letter.
 *
 * Bounded well below NUT's loop, which can block for 100 s: this runs in
 * ups_task, and a stuck poll holds up the command queue and the recovery
 * logic behind it. Three sends, each read up to three times. */
static bool tl_xfer(const char *msg, size_t len, uint8_t reply[TL_PKT])
{
    uint8_t pkt[TL_PKT];
    tl_build_cmd((const uint8_t *)msg, len, pkt);
    for (int send = 0; send < 3; send++) {
        if (!ctrl_out(0x21, REQ_SET_REPORT, (uint16_t)(REPORT_TYPE_FEATURE << 8),
                      s_hid_iface, pkt, TL_PKT)) {
            continue;
        }
        vTaskDelay(pdMS_TO_TICKS(100));     /* NUT: replies take 80-90 ms */
        for (int rx = 0; rx < 3; rx++) {
            uint8_t buf[64];
            int n = intr_in(buf, sizeof(buf), 400);
            if (n >= TL_PKT && tl_reply_is_for(pkt, buf)) {
                memcpy(reply, buf, TL_PKT);
                return true;
            }
        }
    }
...
/* Legacy enumeration: which protocol, watchdog off, configuration. */
static bool tl_enumerate(ups_data_t *local)
{
    if (!s_intr_ep) {
        ESP_LOGE(TAG, "legacy Tripp Lite without an interrupt IN endpoint");
        return false;
    }
    if (!s_intr_done) s_intr_done = xSemaphoreCreateBinary();
    if (!s_intr_done) return false;

    uint8_t r[TL_PKT];
    if (!tl_xfer("\0", 2, r)) {
        ESP_LOGE(TAG, "legacy Tripp Lite: no answer to the protocol query");
        return false;
    }
    uint16_t proto = tl_protocol_from_reply(r);
    tl_config_init(&s_tl, tl_decode_protocol(proto));
    ESP_LOGI(TAG, "legacy Tripp Lite, protocol %04x: %s", proto, tl_model_name(s_tl.model));

    /* The watchdog powers the load off if nothing talks to the UPS in time.
     * NUT turns it off on every model but SMARTPRO, which has none. */
    if (s_tl.model != TL_SMARTPRO && !tl_xfer("W\0", 3, r)) {
        ESP_LOGW(TAG, "legacy Tripp Lite: could not turn the watchdog off");
    }
    if (tl_xfer("V", 2, r)) tl_decode_v(&s_tl, r);
    ESP_LOGI(TAG, "legacy Tripp Lite: %d V input, %d V battery",
             s_tl.input_nominal, s_tl.battery_nominal);

    if (!local->mfr[0]) strlcpy(local->mfr, "Tripp Lite", sizeof(local->mfr));
    local->caps = 0;        /* no controls ported: tests and shutdown stay NUT-only */
    s_legacy = true;
    return true;
}

/* One legacy poll into d. False only when the UPS stops answering; an
 * unrecognised status code keeps the readings and reports the status as
 * unknown, rather than counting as a failure -- which would send a unit that
 * always says it into the VBUS recovery loop. */
static bool tl_poll(ups_data_t *d)
{
    char cmds[4];
    size_t n = tl_poll_commands(s_tl.model, cmds);
    tl_reading_t rd;
    tl_reading_init(&rd);
    for (size_t i = 0; i < n; i++) {
        char msg[2] = { cmds[i], 0 };
        uint8_t r[TL_PKT];
        if (!tl_xfer(msg, 2, r)) return false;
        switch (cmds[i]) {
        case 'S': tl_decode_s(&s_tl, r, &rd); break;
        case 'B': tl_decode_b(&s_tl, r, &rd); break;
        case 'D': tl_decode_d(&s_tl, r, &rd); break;
        case 'T': tl_decode_t(&s_tl, r, &rd); break;
        case 'L': tl_decode_l(&s_tl, r, &rd); break;
        default: break;
        }
    }
    ...
    uint32_t st = 0;
    if (rd.status & TL_ST_OL)   st |= UPS_STATUS_ONLINE;
    if (rd.status & TL_ST_OB)   st |= UPS_STATUS_ONBATT | UPS_STATUS_DISCHARGE;
    if (rd.status & TL_ST_LB)   st |= UPS_STATUS_LOWBATT;
    if (rd.status & TL_ST_RB)   st |= UPS_STATUS_REPLACEBATT;
    if (rd.status & TL_ST_OVER) st |= UPS_STATUS_OVERLOAD;
    d->status = rd.stale ? UPS_STATUS_UNKNOWN : st;

    d->battery_charge  = rd.battery_charge;
    d->battery_voltage = rd.battery_voltage;
    d->input_voltage   = rd.input_voltage;
    d->input_frequency = rd.input_frequency;
    d->output_voltage  = rd.output_voltage;
    d->ups_load        = rd.load;
    return true;
}
...
static bool enumerate(void)
{
    ...
    s_legacy = false;
    if (local.vid == TL_VID && local.pid == TL_LEGACY_PID) {
        if (!tl_enumerate(&local)) return false;
        local.attached = true;
...
static void ups_task(void *arg)
{
    ...
    for (;;) {
        ...
        ups_data_t fresh;
        if (have_dev && (s_poll_count || s_legacy)) {
            xSemaphoreTake(s_lock, portMAX_DELAY);
            fresh = s_data;                 /* keep identity and link state */
            xSemaphoreGive(s_lock);
            ok = s_legacy ? tl_poll(&fresh) : poll_values(&fresh);
        }
...
{
    /* Data first, lock second: ups_hid_get() treats a NULL lock as "not
     * started", so once the lock exists the data behind it must be valid. */
    ups_data_reset(&s_data);
...
{
    /* Callers start before this component does -- the web UI and the display
     * both come up ahead of ups_hid_start() in app_main, and the display's
     * first refresh used to take a lock that did not exist yet and abort.
     * Before start there is no UPS and nothing measured, so say exactly that. */
    if (!s_lock) {
        ups_data_reset(out);
        return;
    }
...
static esp_err_t do_recover(void)
{
    ...
    /* Pump events briefly so a disconnect caused by the cycle lands now, and
     * the loop's DEV_GONE teardown -- not the code below -- owns that case. */
    int64_t until = esp_timer_get_time() + (int64_t)VBUS_GONE_WAIT_MS * 1000;
    while (!s_dev_gone && esp_timer_get_time() < until) {
        usb_host_client_handle_events(s_client, pdMS_TO_TICKS(100));
    }

    /* A SELF-POWERED UPS DOES NOT LEAVE. Its USB interface runs from its own
     * supply, so dropping VBUS does not disconnect it: no DEV_GONE, no
     * NEW_DEV. The map was then wiped below and nothing ever rebuilt it,
     * because only enumeration does -- the UPS stayed attached and was never
     * polled again, until someone replugged it by hand. The first Rev A board
     * did this to an APC Back-UPS RS 1000G on 2026-09-25, from both the
     * page's button and the automatic recovery; the devkit has no load switch
     * and could never have shown it.
     *
     * So if it is still here, close it and enumerate it again at the same
     * address: a fresh descriptor read, the map rebuilt, and whatever state
     * the interface was wedged in is at least re-requested from scratch. */
    if (s_dev && !s_dev_gone) {
        ESP_LOGW(TAG, "UPS stayed connected through the VBUS cycle "
                      "(self-powered), re-enumerating it in place");
        usb_host_interface_release(s_client, s_dev, s_hid_iface);
        usb_host_device_close(s_client, s_dev);
        s_dev = NULL;
        s_hid_iface = 0xFF;
        s_legacy = false;
        s_dev_pending = true;          /* s_dev_addr is unchanged */
    }
