...
static esp_err_t get_status(httpd_req_t *req)
{
     ...
     *
     * Counted once per boot, on the first request served, and only for the
     * reasons that mean something went wrong -- not for an ordinary power-on
     * or a deliberate restart after an update. */
    static bool counted;
    ...
    n += snprintf(buf + n, sizeof(buf) - n,
                  ...
                  "\"pending\":%s,\"auto\":%s,\"checked\":%s,\"error\":\"%s\","
                  "\"refused\":\"%s\"}}",
...
/* The setup form. Replies BEFORE the device reboots, which is why
 * netmgr_provision defers the restart rather than doing it inline: a page that
 * says "saved" and then loses the socket is correct, a page that reports a
 * failure for something that worked is not. */
 ...
 *     curl -O http://ups-adaptor.local/api/descriptor
 ...
 * front of it with the BOOT button held at power-on -- see app_main. Firmware
...
esp_err_t webui_start(void)
{
    ...
    /* The default is 8 and this serves 12. Exceeding it used to abort inside
    ...
    if (netmgr_state() == NETMGR_AP_PROVISIONING) {
        ...
        ESP_LOGI(TAG, "web UI on port 80, http://ups-adaptor.local/");
    }
    return ESP_OK;
}
