...
/* The latest release's notes, abridged: its "What changed" bullets as
 * newline-separated items (at most six, each cut to fit), and the release
 * page's URL for the rest. Empty until a check has succeeded, and empty notes
 * when the release body has no such list. Kept out of ota_status_t so the
 * status page's per-poll copy does not carry a kilobyte it rarely needs. */
