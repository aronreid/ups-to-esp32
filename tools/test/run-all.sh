...
echo
echo "── firmware: builds for both targets"
if [[ -n "${IDF_PATH:-}" ]]; then
    ok=1
    for board in REV_A FREENOVE; do
        ( cd "$root/firmware" \
          && sed -i '' "s/^CONFIG_UPSA_BOARD_.*=y/CONFIG_UPSA_BOARD_${board}=y/" sdkconfig 2>/dev/null
          idf.py build >/tmp/build-$board.log 2>&1 ) \
          && echo "  ok   $board builds" || { echo "  FAIL $board (see /tmp/build-$board.log)"; ok=0; }
    done
