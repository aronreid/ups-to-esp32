...
```
$ nc ups-esp32-XXXX.local 3493
LIST VAR ups
BEGIN LIST VAR ups
VAR ups device.type "ups"
VAR ups driver.name "ups-esp32"
VAR ups ups.mfr "CPS"
...
| **Prometheus** | Scrape `http://ups-esp32-XXXX.local/metrics` directly — no exporter in between. Metric names follow `nut_exporter` (`network_ups_tools_battery_charge`, `network_ups_tools_ups_status{flag="OB"}`, …), so its dashboards and alerts carry over. |
...
- **Wi-Fi setup in a browser.** First boot raises a `ups-esp32-XXXX` access point with
...
- **An optional OLED.** A 4-pin header for a 0.96" 128×64 SSD1306 module, for charge,
  runtime, input volts and load at a glance without opening a browser. **On Rev A, read
  the warning under *The board* before fitting one.**
- **mDNS** — `ups-esp32-XXXX.local`, unique per board and matching its setup access point, advertising both `_http._tcp` and `_nut._tcp`.
...
printed case (`tools/gen-case.py`). The views here are renders of the design.

### The case

Printable, no supports, no screws needed to close it: the board drops into the tray,
and the cover snaps on and holds it down. STLs are in
[`hardware/enclosure/`](hardware/enclosure/), generated by `tools/gen-case.py`.
...
| `ups-adaptor-cover-oled.stl` | a taller cover with a window, for a board with the display fitted |
| `ups-adaptor-lightpipes.stl` | two 3 mm light pipes, printed in clear PETG |

Either tray takes either cover. Print the cover upside down, as the STL is oriented.

### Schematic

[![the Rev A schematic, click for the PDF](docs/img/schematic.svg)](docs/schematic.pdf)

[PDF](docs/schematic.pdf) · [SVG](docs/img/schematic.svg) · KiCad source in
[`hardware/kicad/`](hardware/kicad/). Generated by `tools/gen-schematic.py` from the same
netlist the board is built from, so it cannot drift from the hardware; CI fails if it
has changed without being re-exported.
...
>
> Either way the display needs a firmware built with
> `CONFIG_UPSA_REVA_OLED_VCC_FIRST=y`, which swaps SDA and SCL to match. Release firmware
> leaves the display off on Rev A. **Before first power, read the labels on the module in
> your hand and put VCC on J3's square pad.** Rev B's header takes a standard module as
> seated.
...
one and brings the setup access point back. Point a NUT client at port 3493, UPS name `ups`, no
credentials.
